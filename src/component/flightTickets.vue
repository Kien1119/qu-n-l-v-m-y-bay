...
<template>
  <div class="flex flex-col gap-5 pb-20">
        ...
        <div class="border-2 rounded-lg">
          <div class="flex gap-3 bg-gradient-to-r from-cyan-500 from-10%">
            <div class="h-ful flex justify-center items-center bg-cyan-200">
              <svg data-v-532b2a7a="" xmlns="http://www.w3.org/2000/svg" width="50px" height="30px" viewBox="0 0 24 24"
                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                class="text-body feather feather-dollar-sign">
                <line data-v-532b2a7a="" x1="12" y1="1" x2="12" y2="23"></line>
                <path class="w-50px" data-v-532b2a7a="" d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path>
              </svg>
            </div>
          ...
          <div class="card border-2 !border-gray-500 !rounded-lg mx-5 my-5">
            <DataTable :value="price" tableStyle="min-width: 50rem ">
              ...
              <template #footer>
                <span class="text-red-400 font-bold w-full flex flex-row-reverse">Giá chi tiết:
    ...
    <!-- form nhập thông tin -->
    <Form :initial-values="initialValues" :validation-schema="schema" class="flex flex-col gap-5" @submit="getHold">
      <!-- Thông tin hành khách -->
      <div class="border-2 rounded-lg bg-slate-50">
        <div class="flex gap-3 bg-gradient-to-r from-cyan-500 from-10%">
          <div class="flex justify-center items-center bg-cyan-200">
            <svg data-v-33ccfcdd="" xmlns="http://www.w3.org/2000/svg" width="50px" height="30px" viewBox="0 0 24 24"
              fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
              ...
              <div v-for="(field, idx) in fields" :key="field.key" class="card p-3 rounded-lg">
                ...
                <div class="grid grid-cols-2 gap-6">
                  ...
                  <div class="flex flex-col gap-1 mb-3">
                    <label :for="`firstName_${idx}`">Họ <span class="text-red-600">(*)</span></label>
                    <Field :id="`firstName_${idx}`" :name="`paxLists[${idx}].firstName`"
                      class="px-2 w-full uppercase h-10 rounded-md" placeholder="Họ" as="input" />
                    <ErrorMessage :name="`paxLists[${idx}].firstName`" class="text-red-600" />
                  </div>

                  <!-- Trường Tên -->
                  <div class="flex flex-col gap-1">
                    <label :for="`lastName_${idx}`">Tên đệm & Tên <span class="text-red-600">(*)</span></label>
                    <Field :id="`lastName_${idx}`" :name="`paxLists[${idx}].lastName`"
                      class="px-2 w-full uppercase h-10 rounded-md" placeholder="Tên đệm & Tên" as="input" />
                    <ErrorMessage :name="`paxLists[${idx}].lastName`" class="text-red-600" />
                  </div>

                  <!-- Trường Danh xưng -->
                  <div class="flex flex-col gap-1">
                    <label :for="`titleName_${idx}`">Danh xưng<span class="text-red-600">(*)</span></label>
                    <Field :id="`titleName_${idx}`" :name="`paxLists[${idx}].titleName`" as="select"
                      placeholder="Danh xưng" class="w-full h-10 rounded-md">
                  ...
                  <!-- Trường Ngày sinh -->
                  <div class="flex flex-col gap-1">
                    <label :for="`birthday_${idx}`">Ngày sinh<span class="text-red-600">(*)</span></label>
      ...
      <div class="border-2 rounded-lg bg-slate-50">
        ...
        <div class="flex card border-2 rounded-lg mx-5 my-5 gap-3 bg-slate-100">
          <div class="flex-1 flex flex-col gap-1 p-3">
            ...
            <Field name="contact.email" v-model="initialValues.contact.email" type="email" placeholder="Email"
          ...
          <div class="flex-1 flex flex-col gap-1 p-3">
            ...
            <Field name="contact.phone" v-model="initialValues.contact.phone" type="tel" placeholder="Số điện thoại"
              class="w-full h-10 rounded-md px-3" />
      ...
      <!-- Thao tác -->
      <div class="backdrop-blur-md flex !bg-white/50 rounded-2xl border justify-center fixed inset-x-0 bottom-0">
        <div class="py-3 flex gap-3">
          <Button class="" label="Quay lại" @click="backBooking"></Button>
          <Button class="" label="Giữ chỗ" @click="holdBooking"></Button>
          <Button class="" label="Check chỗ" type="submit"></Button>
        </div>
      </div>
    </Form>
...
const { remove, push, fields } = useFieldArray('paxLists');
